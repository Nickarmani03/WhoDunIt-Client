  ...
  const handleRegister = (e) => {
    ...
    if (password.current.value === verifyPassword.current.value) {
      ...
      return fetch("http://127.0.0.1:8000/register", {
        ...
        .then(res => {
          if ("token" in res) {
            localStorage.setItem("whodunit_token", res.token)
            history.push("/")
          }
        })
    } else {
    ...
    <main style={{ textAlign: "center" }}>
      ...
      <form className="form--login" onSubmit={handleRegister}>
        ...
        <fieldset>
          ...
          <input ref={username} type="text" name="userName" className="form-control" placeholder="Choose a user name" required />
        </fieldset>
